            ...
            <div className="form-container-room">
                <form onSubmit={handleSaveChanges}>
                    <div className="form-group">
                        ...
                        <input type="text" id="name" name="name" required value={room.name || ''} onChange={e => setRoom({...room, name: e.target.value})}/>
                    </div>
                    <div className="form-group">
                        <label htmlFor="capacity">Capacity:</label>
                        <input type="text" id="capacity" name="capacity" required value={room.capacity || ''} onChange={e => setRoom({...room, capacity: e.target.value})}/>
                    </div>
                    <div className="form-group">
                        <label htmlFor="floor">Floor:</label>
                        {/* different logic so we can have floor=0 */}
                        <input type="text" id="floor" name="floor" required value={room.floor !== undefined && room.floor !== null ? room.floor : ''} onChange={e => setRoom({...room, floor: e.target.value})}/>
